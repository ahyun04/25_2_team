...
public class NetworkManager : MonoBehaviour, INetworkRunnerCallbacks
{
    ...
    private void OnGUI()
    {
        ...
        {
            GUILayout.BeginArea(new Rect(10, 10, 300, 300));
            if (GameMultiManager.Instance != null && GameMultiManager.Instance.IsGameStarted)
            {
            ...
            else
            {
                if (GUILayout.Button("Ready"))
                {
                    ...
                    if (GameMultiManager.Instance != null)
                    {
                        GameMultiManager.Instance.RPC_PlayerReady(_runner.LocalPlayer);
                    }
                    else
                    {
                        Debug.LogError("GameMultiManager instance is null");
                    }
                }
                if (GameMultiManager.Instance != null)
                {
                    GUILayout.Label($"현재 플레이어 수: {GameMultiManager.Instance.PlayerCount}");
                    GUILayout.Label($"준비 상태: {GameMultiManager.Instance.GetPlayerState(_runner.LocalPlayer)}");
                }
            }
            GUILayout.EndArea();
        }
    ...
    public void OnPlayerJoined(NetworkRunner runner, PlayerRef player)
    {
        ...
        if (GameMultiManager.Instance != null)
        {
            GameMultiManager.Instance.PlayerJoined(player);
        }
    ...
    public void OnPlayerLeft(NetworkRunner runner, PlayerRef player)
    {
        ...
        GameMultiManager.Instance.PlayerLeft(player);
